  ...
  <div v-if="khdata">
    ...
    <div class="detail">
      <div><span class="label">房源:</span>
        {{khdata.presignHouseDetail.propertyName}}
        {{khdata.presignHouseDetail.buildingName ? khdata.presignHouseDetail.buildingName + '栋' : ''}}
        {{khdata.presignHouseDetail.unitName ? khdata.presignHouseDetail.unitName + '单元' : ''}}
        {{khdata.presignHouseDetail.houseName ? khdata.presignHouseDetail.houseName + '室' : ''}}
      </div>
      <div><span class="label">房屋类型:</span>{{khdata.presignHouseDetail.buildType}}</div>
      <div><span class="label">房源价格:</span>{{khdata.presignHouseDetail.price}}元</div>
      <div><span class="label">户型:</span>{{khdata.presignHouseDetail.housewear}}</div>
      <div><span class="label">房源面积:</span>{{khdata.presignHouseDetail.buildArea}}m²</div>
    </div>
    <div class="title">
    ...
    <div class="detail">
      ...
      <div><span class="label">草签面积:</span>{{khdata.presignArea}}m²</div>
      <div v-if="khdata.presignContractImg.length"><span class="label">草签合同:</span>
        <p class="image">
            <span v-for="(item,index) in khdata.presignContractImg" :key="index" @click="imgClick(index,khdata.presignContractImg)">
                 <img :src="imageUrl + item.url + '?imageView2/1/w/103/h/76'"/><em></em>
            </span>
        </p>
      </div>
    ...
    <div class="detail">
      <div><span class="label">团购方案:</span>{{khdata.presignHouseDetail.activityName}}</div>
      ...
      <div><span class="label">应发佣金:</span>{{realHairMoney}}元</div>
...
export default {
  ...
  methods: {
    getkhdata () {
      this.$axios.get(this.jyAppApi + '/presign/' + this.id).then((res) => {
