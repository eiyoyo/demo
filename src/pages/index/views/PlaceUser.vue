      ...
      <section class="section">
        <div class="p-u" v-if="placeUser">
          ...
          <div style="flex: 1">
            ...
            <p class="starScore">
              <img v-for="k in Math.floor(starScore)" :key="k" src="../../../assets/img/yeji_ic_star_yellow.png">
              <img v-if="starScore-Math.floor(starScore) != 0" src="../../../assets/img/yeji_ic_star_half.png">
...
export default {
  ...
  data () {
    return {
      ...
      startTime: decodeURIComponent(this.$route.query.startTime), // 自定义开始时间
      endTime: decodeURIComponent(this.$route.query.endTime), // 自定义结束时间
    ...
    getAllYeji () {
      this.$axios.get(this.sjzxApi + '/stats/user', {
        ...
        if (res.data.code === 200) {
          ...
          this.$refs.Chart1.drawLine(this.type, this.lineData)
          this.loading = false
        }
      })
    }, // 折线图数据
  ...
  mounted () {
    console.log(decodeURIComponent(this.$route.query.startTime))
    this.getPlaceYeji()
    this.getPlaceUser()
    this.getQudao()
    this.getStarScore()
